...
Credentials are validated when collection starts. BMC health states are reported directly; the collector does not
...
|  | retries | Number of extra attempts for a request that fails with a transient error. | 2 | no |
...
| redfish.collection.http_requests | Redfish Collection HTTP Requests | started, retried, redirected | requests/cycle |
